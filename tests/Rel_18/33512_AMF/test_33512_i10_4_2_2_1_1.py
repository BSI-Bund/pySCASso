...
def test_33512_g60_TC_SYNC_FAIL_SEAF_AMF_A(provide_running_core, provide_core_config,
    ...
    LOGGER.info("Nausf_UEAuthentication_Authenticate Response message DATA found")
...
def test_33512_g60_4_2_2_1_1_B(provide_running_core, provide_core_config, provide_running_ran,
    ...
    post = HTTP2HeaderPost(
        additional_filters=[
            DisplayFilter("http2.headers.path", "==", "/nausf-auth/v1/ue-authentications"),
            DisplayFilter("ip.dst", "==", core_config.read(ConfigKey.AUSF_IP))
        ]
    )
    ...
    # Step 1
    assert (cap.get_first_packet_of_type(AuthenticationFailure()))
    ...
    LOGGER.info("HTTP POST /nausf-auth/v1/ue-authentications with AUTS for AUSF found")
    ...
    assert (real_nausf_post_data := cap.get_first_packet_of_type(nausf_post_data))
    ...
    LOGGER.info("HTTP POST Response /nausf-auth/v1/ue-authentications from AUSF not found")

    # Result
    assert (cap.get_first_packet_of_type(RegistrationReject()))
