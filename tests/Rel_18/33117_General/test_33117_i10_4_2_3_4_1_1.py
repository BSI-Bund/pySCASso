...
# authentication attribute. The tester also verifies that the access entries to use system
# functions require authorization via an access control mechanism (e.g. Discretionary access
# control/Ownership/Capabilities or Mandatory access control). This applies to both system
# functions that are locally accessible and those that are remotely accessible via a network
# interface.
#
# Expected Results:
# ----------------
# 1. The network product does not allow access to any system function provided by the manufacturer
# without a successful user authentication and authorization.


def test_33117_i10__TC_SYS_FUN_USAGE():
