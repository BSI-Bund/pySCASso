...
# Source: 33117-g80.md
# Section: 4.2.6.2.3
# Title: GTP-C Filtering
# Purpose:
# To verify that the network product provides filtering functionalities for incoming GTP-C
...
# 2. It is possible to block all GTP-C messages on those network product interfaces where they are
# unwanted.
...
# the network product.
# b. The tester verifies that the GTP-C EchoRequest messages are not discarded because EchoResponse
...
# 7. The tester sends to if1 GTP-C messages different from EchoRequest replying a pcap file or
# using a network generator.
# a. Using the network analyser, the tester verifies that the messages are correctly received by
# the network product.
# b. Using the accounting, the tester verifies that the messages are discarded and that any
# response is sent back by the network product.
# 8. The tester deletes the previous rules and configures a new rule, i.e. to accept only GTP-C
...
# a. Using the network analyser, the tester verifies that the messages are correctly received by
# the network product.
# b. The tester verifies that the GTP-C EchoRequest messages are not discarded and EchoResponse
# messages are sent back by the network product.
# 10. The tester sends GTP-C EchoRequest messages with source IP Address set to IP2 different from
...
# network product.
# b. The tester verifies that the GTP-C EchoRequest messages are discarded and that no EchoResponse
...
# - For steps 8, 9, 10 the tester receives GTP-C EchoResponse messages only for the authorized
# source IP address.


def test_33117_g80_TC_GTP_C_FILTERING():
